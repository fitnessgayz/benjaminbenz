...
Native iOS delivery uses `fwb-native-push` and Apple's APNs provider API directly. `fwb_native_push_queue` receives one job per active iOS device when an in-app notification is created. The worker signs APNs requests with the protected `APNS_TEAM_ID`, `APNS_KEY_ID`, and `APNS_PRIVATE_KEY` Edge Function secrets. Payloads contain generic copy and the notification ID only; message bodies are never sent to APNs. Apple Push Notifications must be enabled for the iPhone App ID and its App Store provisioning profile must be regenerated before the worker and signed build are released. The `fwb-native-push-delivery` cron job is installed inactive and must be activated only after those secrets are configured and a production-device registration is confirmed.
