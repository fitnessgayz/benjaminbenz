...
async function refreshConnection(adminClient: UntypedSupabaseClient, connection: Record<string, unknown>, request: Request) {
...
async function currentConnection(adminClient: UntypedSupabaseClient, email: string, request: Request) {
  const { data, error } = await adminClient
    .from("client_google_health_connections")
    .select("*")
