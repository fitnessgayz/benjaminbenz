...
create or replace function public.delete_client_custom_exercise_history(
  target_client_email text,
  exercise_name_to_delete text
)
returns integer
language plpgsql
security invoker
...
declare
  normalized_email text := lower(btrim(coalesce(target_client_email, '')));
  normalized_exercise_name text := lower(regexp_replace(btrim(coalesce(exercise_name_to_delete, '')), '[[:space:]]+', ' ', 'g'));
  affected_rows integer := 0;
begin
  if normalized_email = '' then
    raise exception 'A client email is required.' using errcode = '22023';
  ...
  if char_length(normalized_exercise_name) < 2 or char_length(normalized_exercise_name) > 120 then
    raise exception 'Choose a valid exercise name to delete.' using errcode = '22023';
  end if;
  ...
  if not (
    ...
    raise exception 'You cannot delete exercise history for this client.' using errcode = '42501';
  end if;

  delete from public.client_workout_logs
  where lower(btrim(client_email)) = normalized_email
    and lower(regexp_replace(btrim(exercise_name), '[[:space:]]+', ' ', 'g')) = normalized_exercise_name
    and lower(btrim(workout_title)) like 'custom workout%'
    and upper(btrim(coalesce(exercise_code, ''))) not in ('WARMUP', 'CARDIO');
...
comment on function public.delete_client_custom_exercise_history(text, text) is
  'Permanently deletes saved custom-workout sets for one exact normalized client exercise label.';

revoke all on function public.delete_client_custom_exercise_history(text, text) from public;
revoke all on function public.delete_client_custom_exercise_history(text, text) from anon;
grant execute on function public.delete_client_custom_exercise_history(text, text) to authenticated;
