...
test("live pages cache-bust every updated deck asset", () => {
  assert.match(dashboard, /css\/style\.css\?v=workout-completion-share-1/);
  assert.match(dashboard, /css\/custom-workout-mobile-fix\.css\?v=group-names-log-action-1/);
  assert.match(dashboard, /js\/script\.js\?v=physical-card-decks-1/);
  assert.match(dashboard, /js\/client-portal\.js\?v=workout-completion-share-1/);
