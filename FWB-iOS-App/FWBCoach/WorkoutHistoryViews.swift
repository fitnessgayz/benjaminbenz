    ...
    private var setNumberBadge: some View {
        HStack(spacing: 8) {
            Text(record.isWarmUp ? "W\(record.warmUpOrdinal ?? 1)" : "\(record.setNumber)")
                ...
                .background(record.isWarmUp ? Color.fwbSurface : Color.fwbAccentFill, in: Rectangle())
                .overlay {
                    if record.isWarmUp { Rectangle().stroke(Color.fwbLime, lineWidth: 1) }
                }
...
private struct WorkoutHistorySetTypeBadge: View {
    ...
    var body: some View {
        Label(setType.title.uppercased(), systemImage: setType.systemImage)
