    ...
    private static func makeExerciseRecords(sessions: [WorkoutHistorySession]) -> [ProgressExerciseRecord] {
        ...
        return grouped.compactMap { id, occurrences in
            ...
            let points = occurrences.compactMap { occurrence -> ProgressExercisePoint? in
                ...
                let bestRecord = occurrence.exercise.records.max { left, right in
                ...
                let maxWeight = occurrence.exercise.records.map(\.weightUsed).max() ?? 0
                let maxReps = occurrence.exercise.records.compactMap(\.reps).max() ?? 0
                let estimatedOneRepMax = occurrence.exercise.records.map { record in
