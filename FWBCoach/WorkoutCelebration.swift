...
enum WorkoutPraiseEvaluator {
    static func strength(
        ...
        let completed = drafts.filter { $0.isCompleted || $0.containsEntry }
    ...
    private static func personalRecordNames(
        ...
        let currentGroups = Dictionary(grouping: current) { normalizedExercise($0.exerciseName, code: $0.exerciseCode) }
        let priorRecords = prior.flatMap(\.records).filter { !$0.isCardio }
        let priorGroups = Dictionary(grouping: priorRecords) { normalizedExercise($0.exerciseName, code: $0.exerciseCode) }

        return currentGroups.compactMap { key, drafts in
            guard let records = priorGroups[key], !records.isEmpty else { return nil }
