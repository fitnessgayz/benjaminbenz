...
private enum ReadinessDateCoding {
...
final class ReadinessSyncStore: ObservableObject {
    ...
    func loadToday(clientEmail: String) async throws -> ReadinessCheckIn? {
        ...
        do {
            ...
            guard let remote = records.first?.checkIn else { return local }
        ...
        for checkIn in queued {
            do {
                try await client
                    .from("client_check_ins")
                    .upsert(
                        ReadinessCheckInPayload(checkIn),
                        onConflict: "client_email,occurred_on"
                    )
                    .execute()
