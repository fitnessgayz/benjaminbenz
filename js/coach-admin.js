    ...
    <article class="coach-workout-exercise" data-coach-workout-exercise>
      ...
      <div class="coach-workout-exercise-grid">
        <label class="coach-workout-exercise-name">
          Exercise
          <input type="text" value="${escapeHtml(values.name || "")}" list="coach-workout-exercise-suggestions" placeholder="Exercise name" autocomplete="off" data-coach-workout-name required />
        </label>
        <label>
          Sets
          <input type="number" value="${escapeHtml(values.sets || 3)}" min="1" max="10" step="1" inputmode="numeric" data-coach-workout-sets required />
        </label>
        <label>
          Reps / range
          <input type="text" value="${escapeHtml(values.reps || "")}" placeholder="15 or 10–12" inputmode="numeric" data-coach-workout-reps required />
        </label>
        <label>
          Weight (lb)
          <input type="text" value="${escapeHtml(values.weight || "")}" placeholder="40 or 45, 75, 75" inputmode="decimal" data-coach-workout-weight required />
        </label>
        <label class="coach-workout-exercise-notes">
          Notes <span>optional</span>
          <textarea rows="2" placeholder="Tempo, setup, or coaching note" data-coach-workout-notes>${escapeHtml(values.notes || "")}</textarea>
        </label>
      </div>
    </article>
  `;
...
function renumberCoachWorkoutExercises() {
  const rows = Array.from(document.querySelectorAll("[data-coach-workout-exercise]"));
  const format = coachWorkoutFormatValue();

  rows.forEach((row, index) => {
    const marker = row.querySelector("[data-coach-workout-marker]");
    const removeButton = row.querySelector("[data-coach-workout-remove]");
...
function renderCoachWorkoutClientOptions() {
  ...
  const currentValue = normalizeEmail(select.value);
  const selectedEmail = normalizeEmail(selectedProgram()?.client_email);
  ...
  const preferredValue = [currentValue, selectedEmail]
    .find((email) => clients.some((program) => normalizeEmail(program.client_email) === email));

  select.value = preferredValue || "";
}

function syncCoachWorkoutClient(program = selectedProgram()) {
  const select = document.getElementById("coach-workout-client");
  const email = normalizeEmail(program?.client_email);

  if (select && email && Array.from(select.options).some((option) => option.value === email)) {
    select.value = email;
  }
}

function renderCoachWorkoutExerciseSuggestions() {
...
async function saveCoachWorkoutLog(event) {
  event.preventDefault();

  const form = document.getElementById("coach-workout-log-form");
  const saveButton = document.getElementById("coach-workout-save");
  const clientEmail = normalizeEmail(document.getElementById("coach-workout-client")?.value);
  const entryDate = document.getElementById("coach-workout-date")?.value || "";

  if (!form?.reportValidity()) {
    coachWorkoutStatus("Complete every required field before saving.", true);
    return;
  }

  if (!clientEmail || !entryDate) {
    coachWorkoutStatus("Choose a client and workout date first.", true);
    return;
  }
  ...
  try {
    ...
    coachWorkoutStatus(error.message || "Check the exercise fields and try again.", true);
    return;
  }

  if (!coachSupabase) {
    coachWorkoutStatus("The coach portal is not connected right now.", true);
    return;
  }

  saveButton.disabled = true;
  coachWorkoutStatus("Saving the workout…");

  try {
    const { data: existingRows, error: existingError } = await coachSupabase
    ...
    const planned = exercises.map((exercise) => {
      ...
      const existingCode = existingCodeByName.get(normalizedName);
    ...
    for (const exercise of planned) {
      const staleSets = existing.filter((row) => (
      ...
      if (staleSets.length > 0) {
        const { error: deleteError } = await coachSupabase
          .from("client_workout_logs")
          .delete()
          .ilike("client_email", clientEmail)
          .eq("entry_date", entryDate)
    ...
    const { error: saveError } = await coachSupabase
      .from("client_workout_logs")
    ...
    await Promise.all([loadTrainingLogsForEmail(clientEmail), loadRecentTrainingLogs()]);
    coachWorkoutStatus(`${rows.length} set${rows.length === 1 ? "" : "s"} saved to ${planned.length} exercise${planned.length === 1 ? "" : "s"} in Custom workout.`);
  } catch (error) {
    coachWorkoutStatus(error.message || "The workout could not be saved. Try again.", true);
  } finally {
    saveButton.disabled = false;
  }
}

function resetCoachWorkoutLogger() {
  const form = document.getElementById("coach-workout-log-form");
  const list = document.getElementById("coach-workout-exercises");
  const clientEmail = document.getElementById("coach-workout-client")?.value || "";
  ...
  if (clientSelect && Array.from(clientSelect.options).some((option) => option.value === clientEmail)) {
    clientSelect.value = clientEmail;
  }
  ...
  coachWorkoutStatus("Choose a client and add the exercises completed today.");
}

function handleCoachWorkoutLogger() {
  const form = document.getElementById("coach-workout-log-form");
  const list = document.getElementById("coach-workout-exercises");
  ...
  document.getElementById("coach-workout-add-exercise")?.addEventListener("click", () => {
    addCoachWorkoutExercise();
    list.querySelector("[data-coach-workout-exercise]:last-child [data-coach-workout-name]")?.focus();
  });
  document.getElementById("coach-workout-reset")?.addEventListener("click", resetCoachWorkoutLogger);
  form.addEventListener("submit", saveCoachWorkoutLog);
  ...
  list.addEventListener("click", (event) => {
    ...
    if (!removeButton || list.querySelectorAll("[data-coach-workout-exercise]").length <= 1) {
      return;
    }
...
function fillForm(program = {}) {
  ...
  syncCoachWorkoutClient(program);
  updateWorkoutSummaries();
